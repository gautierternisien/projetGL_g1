  ...
  loadMissions().then(async () => {
    ...
    if (q) {
      ...
      if (found) {
        ...
        if (['en_cours', 'encours', 'in_progress', 'ongoing'].some(x => st.includes(x))) activeTab.value = 0
        else if (['termine', 'terminee', 'done', 'completed'].some(x => st.includes(x))) activeTab.value = 1
...
function scrollToMission(id: number) {
  ...
  const el = container ? container.querySelector(`#mission-${id}`) as HTMLElement | null : document.getElementById(`mission-${id}`)
    ...
    if (idx !== -1) {
      ...
      fetch(`${API_URL}/missions/${mission.id}`, {
        ...
        .then(async (r) => {
          ...
          await loadMissions()
          await nextTick()
          // if mission still in current tab, scroll to it; otherwise open correct tab
          const found = remoteMissions.value.find((m) => m.id === mission.id)
          if (found) {
            const st = (found.status || 'new').toLowerCase()
            if (['en_cours', 'encours', 'in_progress', 'ongoing'].some(x => st.includes(x))) activeTab.value = 0
            else if (['termine', 'terminee', 'done', 'completed'].some(x => st.includes(x))) activeTab.value = 1
...
const missionsForActiveTab = computed(() => {
  ...
  if (remoteMissions.value.length > 0) {
    const statusMatches = (s?: string, tab = activeTab.value) => {
      ...
      if (tab === 0) return ['en_cours', 'encours', 'in_progress', 'ongoing', 'open'].some(x => norm.includes(x))
      if (tab === 1) return ['termine', 'terminee', 'done', 'completed'].some(x => norm.includes(x))
...
<template>
  <div class="dashboard-wrapper">
    <Header title="Missions" :subtitle="title" />
    ...
    <div class="scrollable-area">
      ...
      <div class="missions-list">
          <Card v-for="m in missionsForActiveTab" :key="m.id" :title="m.title" class="note-card" :id="`mission-${m.id}`">
            <p>{{ m.description }}</p>
          <div class="card-actions">
            <button v-if="activeTab === 2" class="action-btn" @click="moveToInProgress(m)">Commencer</button>
            <button v-if="activeTab === 0" class="action-btn" @click="moveToCompleted(m)">Terminer</button>
            <button v-if="activeTab === 0" class="action-btn warn" @click="moveToNew(m)">Supprimer</button>
            <button v-if="activeTab === 1" class="action-btn" @click="restoreToInProgress(m)">Remettre en cours</button>
          </div>
        </Card>
...
<style scoped>
.dashboard-wrapper { background: white; height: 100vh; display: flex; flex-direction: column; }
.scrollable-area { padding: 100px 20px; overflow-y: auto; flex: 1 }
.category-header { display:flex; gap:12px; align-items:center; margin-bottom:12px }
.emoji-img { font-size: 2.6rem }
.title-block h2 { margin:0 }
.subtitle { margin:0; color: #666 }
.tabs { display:flex; gap:8px; margin:12px 0 18px; align-items:center }
.tab-btn { padding:8px 12px; border-radius:8px; border:none; background:#f0f0f0; cursor:pointer }
.tab-btn.active { background:#679436; color:white }
.missions-list { display:flex; flex-direction:column; gap:12px }
.empty-state { color:#666; text-align:center; margin-top:12px }
.actions { margin-top:20px; display:flex; justify-content:center }
.nav-btn { padding:10px 16px; border-radius:8px; border:none; cursor:pointer }
.prev-btn { background:#f0f0f0 }

/* Actions */
.tabs-actions { margin-left: auto; display:flex; align-items:center }
.back-btn { display:flex; align-items:center; gap:8px; background: white; border: 1px solid rgba(0,0,0,0.06); padding:8px 12px; border-radius:999px; box-shadow: 0 6px 18px rgba(0,0,0,0.06); cursor:pointer; transition: transform 0.08s ease, box-shadow 0.12s ease }
.back-btn:hover { transform: translateY(-2px); box-shadow: 0 10px 24px rgba(0,0,0,0.08) }
.back-icon { font-size: 1.05rem; color: #679436 }
.back-label { font-weight: 700; color: #333 }
.add-btn { background:#679436; color:white; border:none; padding:8px 12px; border-radius:8px; cursor:pointer }
.card-actions { margin-top:10px; display:flex; gap:8px; flex-wrap:wrap }
.action-btn { background:#eee; border:none; padding:6px 10px; border-radius:8px; cursor:pointer }
.action-btn.warn { background:#ffecec; color:#c0392b; border:1px solid #f5c6c6 }
</style>